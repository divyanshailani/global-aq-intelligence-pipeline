...
def load_base_data(conn, country_code):
    ...
    # Load VIIRS Data dynamically per country bounds
    try:
...
def build_v9_4_features(df, viirs, horizon):
    df_v9 = df.copy()
    
    # VIIRS Spatial Join (Blast Radius Engine)
...
def train_v9_4_horizon_model(df_global, viirs, country_code, horizon):
    print(f"\n  ── {country_code}  h={horizon:>2}d (V9.4) ──")
    if len(df_global) < 100: return None

    df_h, features = build_v9_4_features(df_global, viirs, horizon)
    target_col = "value"
...
def main():
    ...
    for cc in COUNTRIES:
        ...
        df_global, viirs = load_base_data(conn, cc)
        
        all_meta[cc] = {}
        for h in HORIZONS:
            meta = train_v9_4_horizon_model(df_global, viirs, cc, h)
            if meta:
                all_meta[cc][f"h{h}"] = meta

    conn.close()
    
    print("\n### V9.4 Global Engine - Final Evaluation")
