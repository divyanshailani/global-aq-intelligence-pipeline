...
python3 "$PIPELINE_DIR/scripts/pipeline/run_daily_collector.py" --incremental-only \
...
for attempt in 1 2 3; do
    if python3 "$PIPELINE_DIR/scripts/pipeline/run_daily_etl.py" --recent-days 5 --max-enrich 300; then
...
python3 "$PIPELINE_DIR/scripts/pipeline/predict_v12_onnx.py" || echo "WARNING: inference returned non-zero"

# ── Step 4: Validate matured forecasts → refresh accuracy.json
echo "Validating past forecasts..."
python3 "$PIPELINE_DIR/scripts/pipeline/validate_predictions.py" || echo "WARNING: validation returned non-zero"
...
if [ -n "$TIMEOUT_BIN" ]; then
    $TIMEOUT_BIN "${BACKFILL_MAX_MIN}m" \
        python3 "$PIPELINE_DIR/scripts/pipeline/run_daily_collector.py" --backfill-only
    rc=$?
    if [ $rc -eq 124 ]; then
