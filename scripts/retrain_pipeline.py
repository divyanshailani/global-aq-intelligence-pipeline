...
def train_horizon_model(conn, country_code, horizon):
    ...
    nmae = test_mae / mean_y_test if mean_y_test > 0 else 0
    accuracy_percentage = max(0.0, (1.0 - nmae) * 100.0)

    print(f"     train R²={train_r2:.4f}  test R²={test_r2:.4f}  "
          f"MAE={test_mae:.2f}  Acc={accuracy_percentage:.1f}%  "
    ...
    if os.path.exists(meta_path):
        with open(meta_path, "r") as f:
            try:
                ...
                old_acc = old_meta.get("metrics", {}).get("accuracy_percentage", -99)
                if old_acc != -99 and accuracy_percentage < (old_acc - 10.0):
                    print(f"     🚨 ABORT: Catastrophic Accuracy collapse (Old: {old_acc:.1f}% → New: {accuracy_percentage:.1f}%)")
                    print(f"     🚨 Retaining old production weights for {country_code} h{horizon}.")
                    return None
            except json.JSONDecodeError:
                pass # Proceed if old meta is corrupted
...
def main():
        ...
        for h in HORIZONS:
            ...
            if cc in all_meta and key in all_meta[cc]:
                ...
                print(
                    ...
                    f"{m['test_mae']:>8.2f}  {m.get('accuracy_percentage', 0):>6.1f}%  "
    ...
    # --- The Ripple Effect ---
    print(f"\n{'═'*62}")
    print(f"  TRIGGERING RIPPLE EFFECT (Predict Pipeline)")
    print(f"{'═'*62}")
    import subprocess
    predict_script = os.path.join(os.path.dirname(__file__), "predict_pipeline.py")
