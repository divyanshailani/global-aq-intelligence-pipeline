"""
Global Air Quality Model — Retrain Pipeline (V11 AOD)
============================================================
This script is triggered by the Admin Dashboard's "Continuous Retraining" button.
It runs the V11 AOD engine training script and then automatically
...
def main():
    print("═" * 62)
    print("  GLOBAL AQ MODEL — RETRAIN PIPELINE (V11 AOD)")
    print("═" * 62)
    ...
    # 1. Train V11 Models
    train_script = os.path.join(scripts_dir, "train_v11_aod_global.py")
    print("\n[1/2] Triggering V11 Model Training...")
    try:
    ...
    try:
        subprocess.run([sys.executable, predict_script, "--skip-fetch"], check=True)
    except subprocess.CalledProcessError as e:
